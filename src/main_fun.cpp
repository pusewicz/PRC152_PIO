...
// 模式标志位
volatile char
    Home_Mode = MAIN_MODE,  // 主页显示模式	//0:常规, 1:大字 2:双守
    Flag_Main_Page = ON,     // 主页显示模式
    SQL_MODE = OFF;         // 长静噪模式

// double STEP_LEVEL[] = {0.0050, 0.0100, 0.0125}; //步进等级
double STEP_LEVEL[] = {0.00500, 0.00625}; // 步进等级
...
u8 RSSI = 0, SC = 3,                                                         // SCERRN CONTRAST  CHAN = 0,
    ...
    ITTS = 4, // ITTS:Idle Time to Sleep

    A20_LEVEL[8] = {0, 20, 40, 60, 80, 100, 150, 255},
    WFM_LEVEL[8] = {0, 5, 15, 30, 50, 75, 120, 255}, //{0, 31, 62, 93, 125, 155, 178, 254},//
    MIC_LEVEL[3] = {64, 165, 255};                  //	MIC_LEVEL[8] = {0, 32, 64,  96, 160, 170, 195, 230};//{0, 32, 64,  96, 160, 170, 195, 230}

int NEED_RESTART = OFF;
...
void VFO_Load_Data(void)
{
    ...
    FM_FREQ = load_FMFreq();
    WFM = OFF;                    // 收音机
    ITTS = load_IdleTime2Sleep(); // 空闲监测时间
...
void VFO_Clear()
{
    ...
    LCD_ShowBattery(100);//保证不会出现空显示的状态
...
void VFO_Refresh()
{
    ...
    case MAIN_MODE:
        ...
        LCD_ShowString0608(26, 2, TRF_SHOW_FIRST[AUD], 1, 38);
        LCD_ShowString0608(38, 2, TRF_SHOW_LAST[MIC], 1, 44);
        break;
...
void mainPage_SwitchChannelDispose()
{
    load_ChannelParameter(chan_arv[NOW].CHAN, &chan_arv[NOW]);
    if (Home_Mode == MAIN_MODE)
        LCD_ShowPIC0608(60, 1, 1, 1);
    else if (Home_Mode == BIG_MODE)
    {
        memset(BIG_MODE_buf, 0, 12);
        LCD_ShowString1016(0, 1, "          ", 1, 120);
        sprintf(BIG_MODE_buf, "%02d-%s", chan_arv[NOW].CHAN, chan_arv[NOW].NN);
        LCD_ShowString1016(0, 1, BIG_MODE_buf, 1, 128);
    }
...
u8 Event_Matrix(u8 matrix_key)
{
    ...
    if (TIMES > 0)
    {
        ...
        if (get_Flag(FLAG_CF_SWITCH_ADDR)) // 频率模式
        {
            ...
            {
                // Serial.printf("chan_arv[NOW].RX_FREQ:%lf\n", chan_arv[NOW].RX_FREQ);
                // Serial.printf("STEP_LEVEL[STEP]:%lf\n", STEP_LEVEL[STEP]);
                // Serial.printf("++Step:%lf\n", chan_arv[NOW].RX_FREQ + STEP_LEVEL[STEP]);
...
int PTT_Control(void) // 按下和松开PTT后处理
{
    if (PTTPress()) //
    {
        ////低电量提醒 禁用PTT
        // Serial.printf("Voltage:%d\n", Get_Battery_Vol());
        if (Get_Battery_Vol() < 15)
        {
            LCD_Clear(GLOBAL32);
            LCD_ShowString0608(37, 2, "LOW POWER", 1, 128);
            LCD_ShowString0608(25, 3, "PLEASE CHARGE", 1, 128);
            delay_ms(1500);
            if (KDU_INSERT)
        ...
        if (KDU_INSERT && Flag_Main_Page)
            readWriteValueToKDU(_ASKALL);
        else
        {
            LCD_ShowAscii0408(0, 0, 'T');
            if (Flag_Main_Page)
    ...
    else
    {
        ...
        if (FLAG_SQ_ONCE == 0)
        {
            if (A002_SQ_READ == 0)
            {
                ...
                reloadTimer(TMR_RSSI_CTRL);
                RSSI = Get_A20_RSSI();
            }
        }
        // 收到信号后每1000ms获取一次信号强度
        if (bsp_CheckTimer(TMR_RSSI_CTRL))
        {
            RSSI = Get_A20_RSSI();
            // Serial.printf("get rssi:%d\n", RSSI);
        }
...
{
    if (KDU_INSERT == ON && Flag_Main_Page == ON) // 规定时间内询问数据超过2次, 152设置为KDU插入模式;
    {
        BackLight_SetVal(BL);
        LCD_Clear(GLOBAL32);
        ...
        {
            FeedDog(); // 喂狗
            if (Encoder_Switch_Scan(0) == key_long)
                SHUT();
            ...
            if (PRC152receiveProcess() == BACK2MAIN)
            {
                KDU_INSERT = OFF;
                TIMES = 0;
                VFO_Clear();
...
void VOL_Reflash(int operate) // 音量设置
{
    ...
    if (bsp_CheckTimer(TMR_VOLT_REFRESH) && PTT_READ && KDU_INSERT == OFF)
    {
        LCD_ShowBattery(Get_Battery_Vol());
    }
}
int MY_GLOBAL_FUN(void) // 全局功能函数
{
    // D_printf("%s\n", __FUNCTION__);
    FeedDog(); // 喂狗
...
void ShortCut_MICGAIN_Select(void) // 主界面快捷设置mic灵敏度
{
    ...
    LCD_ShowString0608(26, 2, TRF_SHOW_FIRST[AUD], 0, 38);
    LCD_ShowString0608(38, 2, TRF_SHOW_LAST[trf], 0, 44);
    trf_old = trf;
    while (1)
    {
        MY_GLOBAL_FUN();
        ...
        case MATRIX_RESULT_CLR:
            ...
            LCD_ShowString0608(38, 2, TRF_SHOW_LAST[trf], 1, 44);
            return;
    ...
    while (1)
    {
        ...
        if (FLAG_Modify)
        {
            ...
            case 6:
                ...
                {
                    RT_NICKNAME_Set(chan_arv[NOW].CHAN, (char *)chan_arv[NOW].NN); //////改了未验证
                    save_ChannelParameter(chan_arv[NOW].CHAN, chan_arv[NOW]);
                    change = 0;
                }
                break;
...
{
    sprintf(matrix_menu1[0][0], "R:%3.5f", chan_arv[NOW].RX_FREQ);
    sprintf(matrix_menu1[0][1], "%s", menu_subvoice[chan_arv[NOW].RS]);
    sprintf(matrix_menu1[1][0], "T:%3.5f", chan_arv[NOW].TX_FREQ);
...
int RT_FREQ_Set(int x, int y, double *result, int vu_mode)
{
    ...
    while (1)
    {
        ...
        if (locate > x && locate < maxLocate + x) // 未设置位设为'_'
            LCD_ShowAscii0608(locate, y, '_', 1);
        if (locate == maxLocate + x || bit == maxBit) // 8个单位设置完了
            flag_finish = 1;
        ...
        if (flag_finish) // 输入完成
        {
            LCD_ShowString0608(x + maxLocate + 6, y, "  ", 1, x + maxLocate + 12);
            if (locate < maxLocate + x || bit < maxBit - 1) // 按下确定键，未写入完毕补零
            {
                for (; bit < maxBit; bit++)
                    freq_buf[bit] = 0;
            }
            ...
            freq = freq + freq_buf[1] * 10 + freq_buf[2] + freq_buf[4] * 0.1 + freq_buf[5] * 0.01 +
                   freq_buf[6] * 0.001 + freq_buf[7] * 0.0001 + freq_buf[8] * 0.00001;
...
int RT_SubVoice_Set(int row, int subvoice) // 第row行显示第subvoice个亚音
{
    ...
    while (1)
    {
        ...
        case MATRIX_RESULT_0:
            subvoice = 0;
            LCD_ShowString0608(74, row, "       ", 1, 128);
            LCD_ShowString0608(74, row, menu_subvoice[0], 0, 128);
            break;
        ...
        if (TIMES > 0)
        {
            ...
            LCD_ShowString0608(74, row, "       ", 1, 128);
            LCD_ShowString0608(74, row, menu_subvoice[subvoice], 0, 128);
        }
        else if (TIMES < 0)
        {
            if (subvoice + TIMES < 0)
...
int RT_GBW_Set(int gbw_temp)
{
    ...
    while (1)
    {
        ...
        if (TIMES != 0)
        {
            ...
            LCD_ShowString0608(92, 1, gbw_temp ? "WIDE" : "NARR", 0, 128);
        }
        switch (Encoder_Switch_Scan(0))
        {
        case key_click:
...
//				收发设置：频道别名
int RT_NICKNAME_Set(unsigned char current_channel, char nn_temp[7])
{
    if (inputString(18, 2, nn_temp, 7, 0))
        return BACK2MAIN;
    return NO_OPERATE;
}
void RT_NICKNAME_SetOLD(unsigned char current_channel, unsigned char nn_temp[7])
{
    TIMES = 0;
    unsigned char
        nn[7] = {32},
...
void RT_CHAN_Switch(void)
{
    ...
    while (1)
    {
        ...
        if (reload)
        {
            reload = 0;
            load_ChannelParameter(chan_temp, &chan_arv[TMP]);
            LCD_ShowString0608(92, 1, chan_arv[TMP].GBW ? "WIDE" : "NARR", 1, 128);
            ...
            LCD_ShowString0608(18, 2, "       ", 1, 128);
            LCD_ShowString0608(74, 2, "       ", 1, 128);
            LCD_ShowString0608(18, 2, (char *)chan_arv[TMP].NN, 1, 128);
            LCD_ShowString0608(74, 2, (char *)name, 0, 128);
        }
        switch (Encoder_Switch_Scan(0))
        {
        case key_click:
...
void Light_Mode_Clear(u8 sel_pos)
{
    ...
    LCD_ShowCenter(2, LIGHT_SHOW, 2, sel_pos);
...
void Light_Mode_Set(void) //
{
    ...
    while (1)
    {
        if (MY_GLOBAL_FUN() == NEED2REFRESH || KDU_INSERT)
        {
            VFO_Clear();
            return;
        }
...
int Zeroize_All(void)
{
    ...
    LCD_ShowCenter(2, CONFIRM_OPT, 2, pos);
...
void Key_Test()
{
    ...
    {
        FeedDog();                                                            // 喂狗
        if (KDU_INSERT || bsp_CheckTimer(TMR_OUT_CTRL) || delay_f1_f2 >= 400) // 100000
        {
            bsp_StopTimer(TMR_OUT_CTRL);
            return;
        }
...
void PGM_Menu()
{
    ...
    while (1)
    {
        ...
        if (ENSURE)
        {
            ENSURE = (*PGM_FUNCTION[num])(num % 3 + 1);
            if (ENSURE == BACK2MAIN)
                return;
            ENSURE = 0;
            LCD_ShowString0408(0, 0, "PGM", 1);
...
int PGM_STEP_Set(u8 row)
{
    ...
    while (1)
    {
        ...
        if (TIMES != 0)
        {
            ...
            // else if (step_temp > 2)
            //     step_temp = 2;
            step_temp = (step_temp + 1) % 2;
            // Serial.printf("current step:%lf\n", STEP_LEVEL[step_temp]);
            LCD_ShowString0608(42, row, STEP_SHOW[step_temp], 0, 128);
            TIMES = 0;
        }
...
int PGM_RTControl(u8 row)
{
    ...
    if (modifyWiFiInfo(WIFI_RT_PROGRAM) == BACK2MAIN)
        return BACK2MAIN;

    KDU_INSERT = ON;
    handleWIFIServer(WIFI_RT_PROGRAM);
    NEED_RESTART = ON;
    KDU_INSERT = OFF;
    return true;
}
//                              相关设置10: 设置空闲关机时间
int PGM_IdleTime2Sleep(u8 row)
{
    TIMES = 0;
    u8 itts_temp = ITTS;
    LCD_ShowAscii0608(78, row, ':', 0);
    LCD_ShowString0608(84, row, IdleHour[ITTS], 0, 108);
    LCD_ShowPIC0608(116, row, 0, 0);
    ...
    while (1)
    {
        ...
        case MATRIX_RESULT_ENT:
            save_IdleTime2Sleep(itts_temp);
            ITTS = itts_temp;
            return ENT2LAST;

        case MATRIX_RESULT_0:
            itts_temp = 0;
            LCD_ShowString0608(84, row, IdleHour[itts_temp], 0, 128);
            break;
