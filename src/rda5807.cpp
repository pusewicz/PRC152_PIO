#include "rda5807.h"

#include "tim_int.h"    //收音机恢复
#include "key.h"        //按键
#include "encoder.h"    //编码器
#include "bsp_m62364.h" //音频控制
#include "FCS152_KDU.h"

#include "bsp_iic.h"
#include "bsp_timer.h"
#include "bsp_MatrixKeyBoard.h"

#include "bsp_storage.h" //储存频率
#include "controller.h"  //使能/控制

#define PURE_FM 2
extern volatile u8 WFM;
extern volatile u8 KDU_INSERT;
extern int TIMES; //编码器旋转次数记录
extern u8 WFM_LEVEL[8], VOLUME, AUD;
extern void SHUT(void);
extern void VOL_Reflash(int operate);
extern int MY_GLOBAL_FUN(void);
extern void VFO_Clear(void);

int handlePureFM_GlobalOP(void);

short RDA5807_ReadReg(unsigned char addr)
{
    short buf;
    IIC_Start();
    IIC_Send_Byte(RDA5807_WRITE);
    IIC_Wait_Ack();
    IIC_Send_Byte(addr);
    IIC_Wait_Ack();
    IIC_Start();
    IIC_Send_Byte(RDA5807_READ);
    IIC_Wait_Ack();
    buf = IIC_Read_Byte(1);
    buf = buf << 8;
    buf = buf | IIC_Read_Byte(0);
    IIC_Stop();
    delay_ms(5);
    return buf;
}
void RDA5807_WriteReg(unsigned char addr, short val)
{
    IIC_Start();
    IIC_Send_Byte(RDA5807_WRITE);
    IIC_Wait_Ack();
    IIC_Send_Byte(addr);
    IIC_Wait_Ack();
    IIC_Send_Byte(val >> 8);
    IIC_Wait_Ack();
    IIC_Send_Byte(val & 0xFF);
    IIC_Wait_Ack();
    IIC_Stop();
}
//

//收音机电源控制开关
void RDA_Power(unsigned char off_on)
{
    if (off_on)
    {
        FM_AMP_EN_SET;
        FM_S_EN_SET;
        return;
    }
    FM_AMP_EN_CLR;
    FM_S_EN_CLR;
}
//

//设置频率,单位为100K，即870*100KHz
void RDA5807_Set_Freq(short freq)
{
    uint16_t timeout = 0;
    RDA5807_WriteReg(RDA5807_R03, ((freq - 870) << 6) + 0x0010);
    while ((RDA5807_ReadReg(RDA5807_R0B) & 0x80) == 0 && timeout++)
        ;
    delay_ms(50);
}
//
//收音机开关， 设置收音机数据时使用
void RDA5807_Init(char off_on)
{
    // Serial.printf("Init FM:%d\n", off_on);
    if (off_on)
    {
        RDA_Power(ON);
        delay_ms(2);
        RDA5807_WriteReg(RDA5807_R02, 0xc001);
        delay_ms(700);
        RDA5807_WriteReg(RDA5807_R04, 0x0400);
        delay_ms(60);
        RDA5807_WriteReg(RDA5807_R05, 0xc5Ac);
        delay_ms(60);
        RDA5807_WriteReg(RDA5807_R02, 0xf281);
        delay_ms(70); // delay_ms(700);

        RDA5807_Set_Freq(load_FMFreq());
        M62364_SetSingleChannel(WFM_LINE_CHAN, WFM_LEVEL[VOLUME]);
        SPK_SWITCH(AUD, ON);
    }
    else
    {
        M62364_SetSingleChannel(WFM_LINE_CHAN, WFM_LEVEL[0]);
        RDA5807_WriteReg(RDA5807_R02, 0x0000);
        delay_ms(50); // printf("Close FM: %d\n", WFM);
        RDA_Power(OFF);
    }
}
//
//收音机开关切换菜单
int RDA5807_Switch(void)
{
    char FM_OLD = WFM;
    char FM_NOW = WFM;
    char flag_GlobalOP = NO_OPERATE;
    TIMES = 0;
    bool ConfirmOP = false;
    LCD_ShowPIC0608(98, 1, 1, 1);
    LCD_ShowString0608(104, 1, FM_MODE[WFM], 0, 128);
    while (1)
    {
        flag_GlobalOP = handlePureFM_GlobalOP();
        if (flag_GlobalOP == BACK2MAIN)
            return BACK2MAIN;
        else if (flag_GlobalOP == ENT2LAST)
            continue;

        if (TIMES != 0)
        {
            if (TIMES > 0)
                FM_NOW = (FM_NOW + 1) % 3;
            else
                FM_NOW = (FM_NOW - 1 + 3) % 3;
            TIMES = 0;
            LCD_ShowString0608(104, 1, FM_MODE[FM_NOW], 0, 128);
        }
        //////////////////////////////////////////////////////////
        switch (Encoder_Switch_Scan(0))
        {
        case key_click:
            ConfirmOP = true;
            break;

        case key_double:
            LCD_ShowAscii0608(98, 1, ' ', 1);
            LCD_ShowString0608(104, 1, FM_MODE[WFM], 1, 128);
            return ENT2LAST;

        case key_long:
            SHUT();
            return NO_OPERATE;
        }
        //////////////////////////////////////////////////////////
        switch (Matrix_KEY_Scan(0))
        {
        case MATRIX_RESULT_P:
        case MATRIX_RESULT_RIGHT:
            TIMES++;
            break;

        case MATRIX_RESULT_N:
        case MATRIX_RESULT_LEFT:
            TIMES--;
            break;

        case MATRIX_RESULT_CLR:
            LCD_ShowAscii0608(98, 1, ' ', 1);
            LCD_ShowString0608(104, 1, FM_MODE[WFM], 1, 128);
            return ENT2LAST;

        case MATRIX_RESULT_ENT:
            ConfirmOP = true;
            break;

        case MATRIX_RESULT_3:
            if (WFM == PURE_FM)
                break;
            return BACK2MAIN;
        }
        //////////////////////////////////////////////////////////
        if (ConfirmOP)
        {
            LCD_ShowAscii0608(98, 1, ' ', 1);
            LCD_ShowString0608(104, 1, FM_MODE[FM_NOW], 1, 128);
            if (FM_NOW == WFM)
                return ENT2LAST;

            WFM = FM_NOW;
            // enter pure FM
            if (WFM == PURE_FM)
            {

                if (FM_OLD == ON)
                {
                    Serial.printf("need to resume\n");
                    RDA5807_ResumeImmediately();   //进入时启动恢复才会用到这个
                    if (RcvSignal() || PTTPress()) //进入时一直接收信号或者按着PTT(后者其实不存在)
                        RDA5807_Init(ON);
                }
                else
                    RDA5807_Init(ON);

                A002_Deinit();
                FM_OLD = PURE_FM;
            }
            else // OFF / ON
            {
                if (FM_OLD == PURE_FM)
                {
                    A002_Init();
                    if ((WFM == ON && A002_SQ_READ == 0) || WFM == OFF)
                        RDA5807_Init(OFF);
                }
                else
                    RDA5807_Init(WFM);
            }

            if (WFM == OFF && A002_SQ_READ && PTT_READ)
                SPK_SWITCH(AUD, OFF);

            if (RDA5807_ReadReg(0xb) & 0x0100)
                LCD_ShowString0608(0, 1, "TRUE       ", 1, 100);
            else
                LCD_ShowString0608(0, 1, "FALSE      ", 1, 100);

            return ENT2LAST;//用于退出编码器选择, 修复显示箭头
        }
    }
}

void Radio_Freq_Show(int fm_freq, int mode)
{
    LCD_ShowAscii1016(0, 2, fm_freq / 1000 % 10 + '0', mode);
    LCD_ShowAscii1016(10, 2, fm_freq / 100 % 10 + '0', mode);
    LCD_ShowAscii1016(20, 2, fm_freq / 10 % 10 + '0', mode);
    LCD_ShowAscii1016(30, 2, '.', mode);
    LCD_ShowAscii1016(40, 2, fm_freq % 10 + '0', mode);
}

//
extern volatile char Home_Mode, Flag_Main_Page;
extern char now_chan;
extern int FM_FREQ;
unsigned char FM_CHAN = 0;

int fm_locate = 0, fm_bit = 0, fm_freq_static;
unsigned char fm_freq_buf[4] = {0, 0, 0, 1}, flag_clear = 0, fm_finish = 0,
              val_in = 0, key_press = 17, first_press = 1;

int FM_Freq_Set_Show(int x, int y, int *result)
{
    key_press = Matrix_KEY_Scan(0);
    if(A002_SQ_READ == 0 && (key_press == MATRIX_RESULT_3 || key_press == MATRIX_RESULT_CLR) )  //用于收到信号时，允许直接退出
        return 8;

    if ( WFM != PURE_FM  && (A002_SQ_READ == 0 || PTT_READ == 0)  )
    {
        // fm_freq_buf[0] = 0, fm_freq_buf[1] = 0, fm_freq_buf[2] = 0, fm_freq_buf[3] = 0;
        memset(fm_freq_buf, 0, 4);
        first_press = 1;
        fm_locate = x;
        fm_bit = 0;
        Radio_Freq_Show(FM_FREQ, 1);
        return NO_OPERATE;
    }

    if (fm_locate == 30 + x)
    {
        LCD_ShowAscii1016(fm_locate, y, '.', 1);
        fm_locate += 10;
    }
    if (fm_locate == 50 + x || fm_bit == 4)
        fm_finish = 1;
    if (fm_locate > x)
        LCD_ShowAscii1016(fm_locate, y, '_', 1);

    
    switch (key_press)
    {
    case MATRIX_RESULT_0:
        if (fm_bit != 0 || (fm_freq_buf[0] == 0 && ((fm_freq_buf[1] == 8 && fm_freq_buf[2] >= 7) || fm_freq_buf[1] == 9)) || fm_freq_buf[0] == 1)
            val_in = 1;
        break;

    case MATRIX_RESULT_1:
        if (WFM == OFF)
            return NO_OPERATE;
        if (fm_bit == 0 || (fm_freq_buf[0] == 0 && ((fm_freq_buf[1] == 8 && fm_freq_buf[2] >= 7) || fm_freq_buf[1] == 9)) || (fm_freq_buf[0] == 1 && fm_freq_buf[1] == 0 && fm_freq_buf[2] < 8 && fm_bit > 1)) // fm_freq_buf已清空为零
            val_in = 1;
        break;

    case MATRIX_RESULT_2:
    case MATRIX_RESULT_4:
    case MATRIX_RESULT_5:
    case MATRIX_RESULT_6:
        D_printf("Press{%d}", key_press);
        if ((fm_freq_buf[0] == 0 && ((fm_freq_buf[1] == 8 && fm_freq_buf[2] >= 7) || fm_freq_buf[1] == 9)) || (fm_freq_buf[0] == 1 && fm_freq_buf[1] == 0 && fm_freq_buf[2] < 8 && fm_bit > 1))
            val_in = 1;
        break;

    case MATRIX_RESULT_3:
        if (fm_bit == 0)
            return 8;

        if ((fm_freq_buf[0] == 0 && ((fm_freq_buf[1] == 8 && fm_freq_buf[2] >= 7) || fm_freq_buf[1] == 9)) || (fm_freq_buf[0] == 1 && fm_freq_buf[1] == 0 && fm_freq_buf[2] < 8 && fm_bit > 1))
            val_in = 1;
        break;

    case MATRIX_RESULT_7:
        D_printf("Press{7}");
        if ((fm_freq_buf[0] == 0 && (fm_freq_buf[1] == 8 || fm_freq_buf[1] == 9)) || (fm_freq_buf[0] == 1 && fm_freq_buf[1] == 0 && (fm_bit == 2 || (fm_freq_buf[2] < 8 && fm_bit > 1))))
            val_in = 1;
        break;

    case MATRIX_RESULT_8:
        D_printf("Press{8}");
        if (WFM == OFF)
            return NO_OPERATE;
        if (fm_bit == 0)
            val_in = 2;
        else if ((fm_freq_buf[0] == 0 && (fm_freq_buf[1] == 8 || fm_freq_buf[1] == 9)) || (fm_freq_buf[0] == 1 && fm_freq_buf[1] == 0 && (fm_bit == 2 || (fm_freq_buf[2] < 8 && fm_bit > 1))))
            val_in = 1;
        break;

    case MATRIX_RESULT_9:
        D_printf("Press{9}");
        if (WFM == OFF)
            return NO_OPERATE;
        if (fm_bit == 0)
            val_in = 2;
        else if ((fm_freq_buf[0] == 0 && (fm_freq_buf[1] == 8 || fm_freq_buf[1] == 9)) || (fm_freq_buf[0] == 1 && fm_freq_buf[1] == 0 && (fm_freq_buf[2] < 8 && fm_bit > 2)))
            val_in = 1;
        break;

    case MATRIX_RESULT_CLR:
        D_printf("Press{CLR}, CLEAR\n");
        if ((fm_freq_buf[0] == 0 && fm_freq_buf[1] == 0 && fm_freq_buf[2] == 0 && fm_freq_buf[3] == 0 && flag_clear) || WFM == OFF)
        {
            flag_clear = 0;
            if (WFM == PURE_FM)
            {
                FM_PageShow();
                return NO_OPERATE;
            }
            return 8; //退出
        }
        LCD_ShowAscii1016(x + 0, y, ' ', 1);
        LCD_ShowAscii1016(x + 10, y, ' ', 1);
        LCD_ShowAscii1016(x + 20, y, ' ', 1);
        LCD_ShowAscii1016(x + 30, y, ' ', 1);
        LCD_ShowAscii1016(x + 40, y, ' ', 1);
        LCD_ShowAscii1016(x + 50, y, '}' + 1, 1); //箭头
        fm_freq_buf[0] = 0, fm_freq_buf[1] = 0, fm_freq_buf[2] = 0, fm_freq_buf[3] = 0;
        fm_locate = x;
        fm_bit = 0;
        flag_clear = 1;  // EXIT需要
        first_press = 1; //清空需要
        break;

    case MATRIX_RESULT_ENT:
        D_printf("Press{ENT}, ENTER\n");
        if (fm_bit > 0 || (fm_bit == 0 && flag_clear == 1))
        {
            if (fm_bit < 3 || fm_locate < 50 + x)
                for (; fm_bit < 4; fm_bit++) //补零
                    fm_freq_buf[fm_bit] = 0;
            fm_finish = 1;
            flag_clear = 0;
        }
        else if (fm_bit == 0)
        {
            LCD_ShowAscii0608(98, 1, ' ', 1);
            if (RDA5807_Switch() == BACK2MAIN)
                return 8;
        }
        break;

    case MATRIX_RESULT_P:
        if (WFM == OFF)
            return 0;
        D_printf("Press{+}\n");
        fm_freq_buf[0] = 0, fm_freq_buf[1] = 0, fm_freq_buf[2] = 0, fm_freq_buf[3] = 0;
        first_press = 1;
        fm_locate = x;
        fm_bit = 0;
        return 2;

    case MATRIX_RESULT_N:
        if (WFM == OFF)
            return 0;
        D_printf("Press{-}\n");
        fm_freq_buf[0] = 0, fm_freq_buf[1] = 0, fm_freq_buf[2] = 0, fm_freq_buf[3] = 0;
        first_press = 1;
        fm_locate = x;
        fm_bit = 0;
        return 3;

    case MATRIX_RESULT_LEFT:
        if (WFM == OFF)
            return 0;
        if (fm_bit != 0)
        {
            for (; fm_bit < 4; fm_bit++)
            {
                fm_freq_buf[fm_bit] = 0;
                LCD_ShowAscii1016(fm_locate, y, '0', 1);
                if (fm_bit == 2)
                    fm_locate += 20;
                else
                    fm_locate += 10;
            }
            first_press = 1;
            fm_locate = x;
            fm_bit = 0;
        }
        D_printf("Press{*}, SEEKING FORWARD\n");
        return 4;

    case MATRIX_RESULT_RIGHT:
        if (WFM == OFF)
            return 0;
        if (fm_bit != 0)
        {
            for (; fm_bit < 4; fm_bit++)
            {
                fm_freq_buf[fm_bit] = 0;
                LCD_ShowAscii1016(fm_locate, y, '0', 1);
                if (fm_bit == 2)
                    fm_locate += 20;
                else
                    fm_locate += 10;
            }
            first_press = 1;
            fm_locate = x;
            fm_bit = 0;
        }
        D_printf("Press[#], SEEKING BACKWARD\n");
        return 5;
    }
    //

    //
    if (val_in == 1)
    {
        flag_clear = 0;
        val_in = 0;
        if (first_press)
        {
            first_press = 0;
            LCD_ShowAscii1016(x + 0, y, ' ', 1);
            LCD_ShowAscii1016(x + 10, y, ' ', 1);
            LCD_ShowAscii1016(x + 20, y, ' ', 1);
            LCD_ShowAscii1016(x + 30, y, ' ', 1);
            LCD_ShowAscii1016(x + 40, y, ' ', 1);
            LCD_ShowAscii1016(x + 50, y, '}' + 1, 1); //箭头
        }
        D_printf("[%d] : %d\n", fm_bit, key_press);
        fm_freq_buf[fm_bit] = key_press;
        fm_bit++;
        LCD_ShowAscii1016(fm_locate, y, key_press + '0', 1);
        fm_locate += 10;
    }
    else if (val_in == 2) // 8x, 9x
    {
        flag_clear = 0;
        val_in = 0;
        if (first_press)
        {
            first_press = 0;
            LCD_ShowAscii1016(x + 0, y, ' ', 1);
            LCD_ShowAscii1016(x + 10, y, ' ', 1);
            LCD_ShowAscii1016(x + 20, y, ' ', 1);
            LCD_ShowAscii1016(x + 30, y, ' ', 1);
            LCD_ShowAscii1016(x + 40, y, ' ', 1);
            LCD_ShowAscii1016(x + 50, y, '}' + 1, 1); //箭头
        }
        D_printf("[2] : %d", key_press);
        fm_freq_buf[0] = 0, fm_freq_buf[1] = key_press;
        fm_bit += 2;
        LCD_ShowAscii1016(fm_locate, y, ' ', 1);
        LCD_ShowAscii1016(fm_locate + 10, y, key_press + '0', 1);
        fm_locate += 20;
    }
    //

    //
    if (fm_finish)
    {
        D_printf("Writing Freq Completed\n");
        LCD_ShowAscii1016(x + 50, y, ' ', 1); //' '

        fm_freq_static = fm_freq_buf[0] * 1000 + fm_freq_buf[1] * 100 + fm_freq_buf[2] * 10 + fm_freq_buf[3];

        if (fm_freq_static < 870 || fm_freq_static > 1090)
            fm_freq_static = 870;

        D_printf("Right Freq : %d*100K\n", fm_freq_static);
        *result = fm_freq_static;
        Radio_Freq_Show(fm_freq_static, 1);

        // fm_freq_buf[0]=0,fm_freq_buf[1]=0,fm_freq_buf[2]=0,fm_freq_buf[3]=0;
        memset(fm_freq_buf, 0, 4);
        fm_freq_static = 0;
        fm_locate = x;
        fm_bit = 0;
        first_press = 1;
        fm_finish = 0;
        return 1;
    }
    //

    return NO_OPERATE;
}
int handlePureFM_GlobalOP(void)
{
    if (WFM == PURE_FM)
    {
        FeedDog();
        VOL_Reflash(VolumeKeyScan(0));
        return NO_OPERATE;
    }
    else
    {
        if (KDU_INSERT)
            return BACK2MAIN;
        if (MY_GLOBAL_FUN() == NEED2REFRESH)
            FM_PageShow();
        if (A002_SQ_READ == 0)
        {
            LCD_ShowString0608(0, 1, "R:", 1, 12);
            if (Home_Mode == DUAL_MODE)
                LCD_ShowFreq(12, 1, now_chan ? chan_arv[CHANB].RX_FREQ : chan_arv[CHANA].RX_FREQ, 1);
            else
                LCD_ShowFreq(12, 1, chan_arv[NOW].RX_FREQ, 1);
        }
        if (PTT_READ == 0)
        {
            LCD_ShowString0608(0, 1, "T:", 1, 12);
            if (Home_Mode == DUAL_MODE)
                LCD_ShowFreq(12, 1, now_chan ? chan_arv[CHANB].TX_FREQ : chan_arv[CHANA].TX_FREQ, 1);
            else
                LCD_ShowFreq(12, 1, chan_arv[NOW].TX_FREQ, 1);
            return ENT2LAST;
        }
    }
    return NO_OPERATE;
}
extern int Get_Battery_Vol();
void FM_PageShow(void)
{
    LCD_Clear(GLOBAL32);
    LCD_ShowString0408(0, 0, "R BAT", 1);
    LCD_ShowBattery(Get_Battery_Vol());
    LCD_ShowString0408(48, 0, "VULOS MOI", 1);
    LCD_ShowString0408(88, 0, "------- PT", 1);
    LCD_ShowVolume(VOLUME);

    Radio_Freq_Show(FM_FREQ, 1);
    LCD_ShowString0608(104, 1, FM_MODE[WFM], 1, 128);
    LCD_ShowPIC1616(60, 2, 11, 1);  //<<
    LCD_ShowPIC1616(86, 2, 12, 1);  //>>
    LCD_ShowPIC1616(112, 2, 13, 1); //返回标志
}
void Enter_Radio()
{
    Flag_Main_Page = 0;

    unsigned char
        flag_GlobalOP = NO_OPERATE, //全局函数操作返回标记
        key_encoder,                //编码器返回值
        key_matrix,                 //键盘操作返回值
        locate_encoder = 0,         //编码器第一次按下后,光标所在位置
        flag_return = 0,            //编码器退出操作,清除箭头光标
        fm_change = 1;              //操作后进行频道判断的标志

    FM_PageShow();

    while (1)
    {
        flag_GlobalOP = handlePureFM_GlobalOP();
        if (flag_GlobalOP == BACK2MAIN)
        {
            Serial.println("back2main!");
            return;
        }
        else if (flag_GlobalOP == ENT2LAST)
        {
            Serial.println("A002 OPing!");
            continue;
        }

        key_encoder = Encoder_Switch_Scan(0);
        switch (key_encoder)
        {
        case key_click: //第一次按下编码器
            TIMES = 0;

            if (flag_clear || !first_press)
            {
                if (flag_clear)
                    flag_clear = 0;
                else
                {
                    first_press = 1;
                    fm_freq_buf[0] = 0, fm_freq_buf[1] = 0, fm_freq_buf[2] = 0, fm_freq_buf[3] = 0;
                    first_press = 1;
                    fm_locate = 0;
                    fm_bit = 0;
                }

                Radio_Freq_Show(FM_FREQ, 1);
                LCD_ShowAscii1016(50, 2, ' ', 1);
                break;
            }

            LCD_ShowString0608(98, 1, " ", 1, 128);
            LCD_ShowPIC0608(98, 1, 1, 1);
            LCD_ShowString0608(104, 1, FM_MODE[WFM], 1, 128);
            // Serial.printf("Click: %d#######\n", __LINE__);
            while (1)
            {
                if (handlePureFM_GlobalOP() == BACK2MAIN)
                    return;

                key_matrix = Matrix_KEY_Scan(0);
                key_encoder = Encoder_Switch_Scan(0);

                if (key_matrix == MATRIX_RESULT_P || key_matrix == MATRIX_RESULT_RIGHT)
                    TIMES++;
                if (key_matrix == MATRIX_RESULT_N || key_matrix == MATRIX_RESULT_LEFT)
                    TIMES--;

                if (TIMES != 0) //选择选项
                {
                    if (TIMES > 0) //+
                    {
                        TIMES = 0;
                        locate_encoder = (locate_encoder + 1) % 5;
                    }
                    else if (TIMES < 0) //-
                    {
                        TIMES = 0;
                        locate_encoder = (locate_encoder - 1 + 5) % 5;
                    }
                    LCD_ShowAscii0608(98, 1, ' ', 1);
                    LCD_ShowAscii1016(50, 2, ' ', 1);
                    LCD_ShowAscii1016(77, 2, ' ', 1);
                    LCD_ShowAscii1016(103, 2, ' ', 1);
                    switch (locate_encoder) //箭头显示
                    {
                    case 0:
                        LCD_ShowPIC0608(98, 1, 1, 1);
                        break;
                    case 1:
                        LCD_ShowAscii1016(50, 2, '}' + 1, 1);
                        break;
                    case 2:
                        LCD_ShowAscii1016(77, 2, '}' + 1, 1);
                        break;
                    case 3:
                        LCD_ShowAscii1016(103, 2, '}' + 1, 1);
                        break;
                    case 4:
                        LCD_ShowAscii1016(103, 2, '}' + 2, 1);
                        break;
                    }
                }
                //
                if (key_encoder == key_click || key_matrix == MATRIX_RESULT_ENT)
                {
                    TIMES = 0;
                    switch (locate_encoder)
                    {
                    case 0:
                        LCD_ShowAscii0608(98, 1, ' ', 1);
                        LCD_ShowString0608(104, 1, FM_MODE[WFM], 0, 128);
                        flag_return = RDA5807_Switch();
                        if (flag_return == BACK2MAIN)
                        {
                            VFO_Clear();
                            return;
                        }
                        break;

                    case 1: //按步进调频
                        if (A002_SQ_READ == 0 || WFM == OFF)
                        {
                            flag_return = 1;
                            break;
                        }
                        RDA5807_ResumeImmediately();
                        Radio_Freq_Show(FM_FREQ, 0);
                        while (1)
                        {

                            if (handlePureFM_GlobalOP() == BACK2MAIN)
                                return;
                            if (TIMES != 0)
                            {
                                if (TIMES > 0)
                                {
                                    TIMES--;
                                    if (FM_FREQ < 1080)
                                        FM_FREQ++;
                                }
                                else if (TIMES < 0)
                                {
                                    TIMES++;
                                    if (FM_FREQ > 870)
                                        FM_FREQ--;
                                }
                                RDA5807_Set_Freq(FM_FREQ);
                                Radio_Freq_Show(FM_FREQ, 0);
                                if (RDA5807_ReadReg(0xb) & 0x0100)
                                {
                                    D_printf("fm_freq_temp=%d\n", FM_FREQ);
                                    LCD_ShowString0608(0, 1, "TRUE       ", 1, 100);
                                }
                                else
                                    LCD_ShowString0608(0, 1, "FALSE      ", 1, 100);
                            }
                            //
                            key_encoder = Encoder_Switch_Scan(0);
                            key_matrix = Matrix_KEY_Scan(0);
                            if (key_encoder == key_click || key_encoder == key_double || KDU_INSERT || key_matrix == MATRIX_RESULT_CLR || PTT_READ == 0 || A002_SQ_READ == 0) //确认返回上一级
                            {
                                Radio_Freq_Show(FM_FREQ, 1);
                                save_FMFreq(FM_FREQ);
                                flag_return = 1;
                                break;
                            }
                            else if (key_encoder == key_long)
                                SHUT();
                        }
                        break;

                    case 2: //向下扫频
                        if (A002_SQ_READ == 0 || WFM == OFF)
                        {
                            flag_return = 1;
                            break;
                        }
                        RDA5807_ResumeImmediately();
                        LCD_ShowPIC1616(60, 2, 11, 0);
                        LCD_ShowString0608(0, 1, "SEEKING...   ", 1, 120);
                        while (1)
                        {
                            if (handlePureFM_GlobalOP() == BACK2MAIN)
                                return;

                            FM_FREQ--;
                            if (FM_FREQ < 870)
                                FM_FREQ = 1080;
                            RDA5807_Set_Freq(FM_FREQ);
                            Radio_Freq_Show(FM_FREQ, 1);

                            if (RDA5807_ReadReg(0xb) & 0x0100) //找到台，返回上一级
                            {
                                LCD_ShowString0608(0, 1, "TRUE         ", 1, 120);
                                break;
                            }
                            key_encoder = Encoder_Switch_Scan(0);
                            key_matrix = Matrix_KEY_Scan(0);
                            if (key_encoder == key_click | key_encoder == key_double || key_matrix == MATRIX_RESULT_CLR || KDU_INSERT || PTT_READ == 0 || A002_SQ_READ == 0)
                            {
                                if (RDA5807_ReadReg(0xb) & 0x0100) //找到台，返回上一级
                                    LCD_ShowString0608(0, 1, "TRUE         ", 1, 120);
                                else
                                    LCD_ShowString0608(0, 1, "FALSE        ", 1, 120);
                                break;
                            }
                            else if (key_encoder == key_long)
                                SHUT();
                        }
                        save_FMFreq(FM_FREQ);
                        LCD_ShowPIC1616(60, 2, 11, 1);
                        break;

                    case 3: //向上扫频
                        if (A002_SQ_READ == 0 || WFM == OFF)
                        {
                            flag_return = 1;
                            break;
                        }
                        RDA5807_ResumeImmediately();
                        LCD_ShowPIC1616(86, 2, 12, 0);
                        LCD_ShowString0608(0, 1, "SEEKING...   ", 1, 120);
                        while (1)
                        {

                            if (handlePureFM_GlobalOP() == BACK2MAIN)
                                return;

                            FM_FREQ++;
                            if (FM_FREQ > 1080)
                                FM_FREQ = 870;
                            RDA5807_Set_Freq(FM_FREQ);
                            Radio_Freq_Show(FM_FREQ, 1);

                            if (RDA5807_ReadReg(0xb) & 0x0100) //找到台，返回上一级
                            {
                                LCD_ShowString0608(0, 1, "TRUE         ", 1, 120);
                                break;
                            }
                            key_encoder = Encoder_Switch_Scan(0);
                            key_matrix = Matrix_KEY_Scan(0);
                            if (key_encoder == key_click || key_encoder == key_double || key_matrix == MATRIX_RESULT_CLR || KDU_INSERT || A002_SQ_READ == 0 || PTT_READ == 0)
                            {
                                if (RDA5807_ReadReg(0xb) & 0x0100) //找到台，返回上一级
                                    LCD_ShowString0608(0, 1, "TRUE         ", 1, 120);
                                else
                                    LCD_ShowString0608(0, 1, "FALSE        ", 1, 120);
                                break;
                            }
                            else if (key_encoder == key_long)
                                SHUT();
                        }
                        save_FMFreq(FM_FREQ);
                        LCD_ShowPIC1616(86, 2, 12, 1);
                        break;

                    case 4: //退出收音机
                        if (WFM == PURE_FM)
                            break;
                        VFO_Clear();
                        return;
                    }
                    TIMES = 0;
                }
                else if (key_encoder == key_double || key_matrix == MATRIX_RESULT_CLR ||
                         ((PTT_READ == 0 || A002_SQ_READ == 0) && WFM != PURE_FM))
                    flag_return = 1;
                else if (key_encoder == key_long)
                    SHUT();
                if (flag_return) //退出编码器选择
                {
                    LCD_ShowAscii0608(98, 1, ' ', 1);
                    LCD_ShowAscii1016(50, 2, ' ', 1);
                    LCD_ShowAscii1016(77, 2, ' ', 1);
                    LCD_ShowAscii1016(103, 2, ' ', 1);
                    flag_return = 0;
                    break;
                }
            }
            locate_encoder = 0;
            break;

        case key_double:
            if (flag_clear || !first_press)
            {
                if (flag_clear)
                    flag_clear = 0;
                else
                {
                    first_press = 1;
                    fm_freq_buf[0] = 0, fm_freq_buf[1] = 0, fm_freq_buf[2] = 0, fm_freq_buf[3] = 0;
                    first_press = 1;
                    fm_locate = 0;
                    fm_bit = 0;
                }

                Radio_Freq_Show(FM_FREQ, 1);
                LCD_ShowAscii1016(50, 2, ' ', 1);
                break;
            }
            if (WFM == PURE_FM)
                break;
            VFO_Clear();
            return;

        case key_long:
            SHUT();
            return;
        }

        // Serial.printf("#######%s: %d#######\n", __FUNCTION__, __LINE__);
        key_matrix = FM_Freq_Set_Show(0, 2, &FM_FREQ);
        if (key_matrix == 1) //写完
        {
            fm_change = 1;
            RDA5807_Set_Freq(FM_FREQ);
        }
        else if (key_matrix == 2 || TIMES > 0) //上加  p键
        {
            TIMES = 0;
            if (WFM == OFF)
                continue;

            RDA5807_ResumeImmediately();
            fm_change = 1;
            if (FM_FREQ < 1080)
                FM_FREQ++;

            RDA5807_Set_Freq(FM_FREQ);
            Radio_Freq_Show(FM_FREQ, 1);
        }
        else if (key_matrix == 3 || TIMES < 0) //下减  N键
        {
            TIMES = 0;
            if (WFM == OFF)
                continue;
            RDA5807_ResumeImmediately();
            fm_change = 1;
            if (FM_FREQ > 870)
                FM_FREQ--;

            RDA5807_Set_Freq(FM_FREQ);
            Radio_Freq_Show(FM_FREQ, 1);
        }
        else if (key_matrix == 4) //下扫 左键
        {
            TIMES = 0;
            RDA5807_ResumeImmediately();
            LCD_ShowPIC1616(60, 2, 11, 0);
            LCD_ShowString0608(0, 1, "SEEKING...   ", 1, 120);
            while (1)
            {
                if (handlePureFM_GlobalOP() == BACK2MAIN)
                    return;
                FM_FREQ--;
                if (FM_FREQ < 870)
                    FM_FREQ = 1080;
                RDA5807_Set_Freq(FM_FREQ);
                Radio_Freq_Show(FM_FREQ, 1);

                if (RDA5807_ReadReg(0xb) & 0x0100)
                {
                    save_FMFreq(FM_FREQ);
                    FM_CHAN = 1;
                    LCD_ShowString0608(0, 1, (FM_CHAN ? "TRUE       " : "FALSE      "), 1, 100);
                    break;
                }
                key_encoder = Encoder_Switch_Scan(0);
                key_matrix = Matrix_KEY_Scan(0);
                if (key_encoder == key_click || key_encoder == key_double || key_matrix == MATRIX_RESULT_CLR || KDU_INSERT ||
                    ((A002_SQ_READ == 0 || PTT_READ == 0) && WFM == ON))
                {
                    fm_change = 1;
                    break;
                }
                else if (key_encoder == key_long)
                    SHUT();
            }
            //
            TIMES = 0;
            LCD_ShowPIC1616(60, 2, 11, 1);
        }
        else if (key_matrix == 5) //上扫 右键#
        {
            TIMES = 0;
            RDA5807_ResumeImmediately();
            LCD_ShowPIC1616(86, 2, 12, 0);
            LCD_ShowString0608(0, 1, "SEEKING...   ", 1, 120);
            while (1)
            {
                if (handlePureFM_GlobalOP() == BACK2MAIN)
                    return;
                FM_FREQ++;
                if (FM_FREQ > 1080)
                    FM_FREQ = 870;
                RDA5807_Set_Freq(FM_FREQ);
                Radio_Freq_Show(FM_FREQ, 1);

                if (RDA5807_ReadReg(0xb) & 0x0100)
                {
                    save_FMFreq(FM_FREQ);
                    FM_CHAN = 1;
                    LCD_ShowString0608(0, 1, (FM_CHAN ? "TRUE       " : "FALSE      "), 1, 100);
                    break;
                }
                key_encoder = Encoder_Switch_Scan(0);
                key_matrix = Matrix_KEY_Scan(0);
                if (key_encoder == key_click || key_encoder == key_double || key_matrix == MATRIX_RESULT_CLR || KDU_INSERT ||
                    ((A002_SQ_READ == 0 || PTT_READ == 0) && WFM == ON))
                {
                    fm_change = 1;
                    break;
                }
                else if (key_encoder == key_long)
                    SHUT();
            }
            TIMES = 0;
            LCD_ShowPIC1616(86, 2, 12, 1);
        }
        else if (key_matrix == 8) //退出 CLR键
        {
            if (WFM == PURE_FM)
                continue;
            VFO_Clear();
            return;
        }
        D_printf("#######%s: %d#######\n", __FUNCTION__, __LINE__);

        //判断真台与否
        if (fm_change)
        {
            fm_change = 0;
            D_printf("#######%s: %d#######\n", __FUNCTION__, __LINE__);
            save_FMFreq(FM_FREQ);
            D_printf("#######%s: %d#######\n", __FUNCTION__, __LINE__);
            FM_CHAN = RDA5807_ReadReg(0xb) & 0x0100;
            LCD_ShowString0608(0, 1, FM_CHAN ? "TRUE       " : "FALSE      ", 1, 100);
        }
    }
}
//
void test_FM(void)
{
    while (1)
        Enter_Radio();
}